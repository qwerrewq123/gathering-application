...
public interface ChatRoomRepository extends JpaRepository<ChatRoom, Long> {
    @Query("select new spring.myproject.dto.response.chat.query." +
            ...
            "where cp.chatRoom.id = c.id and r.status=false),g.title," +
            "(select cm1.content from ReadStatus r1 " +
            "left join r1.chatParticipant cp1 " +
            "left join r1.chatMessage cm1 " +
            "where r1.status = false and cp1.chatRoom.id = c.id order by r1.id desc limit 1)," +
            "(select cm2.createdAt from ReadStatus r2 " +
            "left join r2.chatParticipant cp2 " +
            "left join r2.chatMessage cm2 " +
            "where r2.status = false and cp2.chatRoom.id = c.id order by r2.id desc limit 1)) " +
    ...
    @Query("select new spring.myproject.dto.response.chat.query." +
            ...
            "(select count(r.id) from ReadStatus r " +
            "left join r.chatParticipant cpp where cpp.chatRoom.id = c.id and r.status=false) " +
            "else 0 end," +
            "g.title," +
            "(select cm1.content from ReadStatus r1 " +
            "left join r1.chatParticipant cp1 " +
